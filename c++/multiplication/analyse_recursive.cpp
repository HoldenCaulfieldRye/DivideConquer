...
int main( int argc, char *argv[] )
{
	...
	if(datfile)
	{
		...
		std::cout << std::endl;
		std::cout << "Analysing recursive multiplication..." << std::endl;
		...
		for (int d = 0; d < number_types; d++)
		{
			...
			for (int i = 0; i < numbers_per_type; i++)
			{
				for (int j = 0; j < numbers_per_type; j++)
				{
					...
					long long z = RecursiveMultiplication::multiply(x, y);
				}
			}
		...
		resfile.open( "timings_recursive.txt" );
		resfile << "# recursive multiplication running time\n"; 
		...
		std::cout << std::endl;
		std::cout << "Done. Results saved in timings_recursive.txt." << std::endl;
		std::cout << std::endl;
	}
	else
	{
